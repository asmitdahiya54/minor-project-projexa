    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            if user["role"] == "student" and user["student_id"]:
                ...
                session["display_name"] = user["username"].title()
...
def mark_attendance():
    ...
    conn = db()
    visible_ids = {r["id"] for r in visible_students({})}
    saved = 0
    alerts = []
    ...
    for key, value in request.form.items():
        if not key.startswith("att_"):
            continue
        sid = to_int(key.replace("att_", ""))
        if sid not in visible_ids or value not in {"Present", "Absent", "Late"}:
            continue
        conn.execute(
            """
            ...
            """,
            (sid, att_date, value, subject),
        )
        saved += 1
        summary = attendance_summary(sid)
        if summary["total"] and summary["rate"] < 75:
            student = conn.execute("SELECT name,email FROM students WHERE id=?", (sid,)).fetchone()
            if student:
                notify_email(
                    student["email"],
                    "Low attendance alert",
                    f"Hello {student['name']},\n\nYour attendance is {summary['rate']}%, below the 75% threshold.\n\nRegards,\nSIMS Pro"
                )
                alerts.append(student["name"])

    conn.commit()
    msg = f"Attendance saved for {saved} students on {att_date}."
    if alerts:
        msg += f" Low-attendance alerts checked for {', '.join(alerts[:3])}."
    return respond(True, msg, url_for("attendance"))
...
def attendance_report():
    ...
    rows = [{"student": s, "summary": attendance_summary(s["id"])} for s in visible_students(filters)]
